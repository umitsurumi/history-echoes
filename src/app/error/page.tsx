...
function ErrorComponent() {
    ...
    const debugDelay = searchParams.get("debugDelay");
    const [isReady, setIsReady] = useState(!debugDelay);
    useEffect(() => {
        if (debugDelay) {
            const delay = parseInt(debugDelay) || 3000;
    ...
    if (!isReady) {
        throw new Promise((resolve) =>
            setTimeout(resolve, parseInt(debugDelay!) || 3000)
        );
    }

    // 获取错误信息、错误码和重试参数
    const errorMessage =
        searchParams.get("message") || "发生未知错误，请稍后重试。";
