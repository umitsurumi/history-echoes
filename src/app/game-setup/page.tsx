...
export default function GameSetup() {
    ...
    useEffect(() => {
        const savedTimePeriod = localStorage.getItem(
            "history-echoes-timePeriod"
        );
        const savedRegion = localStorage.getItem("history-echoes-region");
        const savedDifficulty = localStorage.getItem(
            "history-echoes-difficulty"
        );

        if (savedTimePeriod) setSelectedTimePeriod(savedTimePeriod);
    ...
    const saveSetting = (key: string, value: string) => {
        localStorage.setItem(`history-echoes-${key}`, value);
    };
    ...
    const timePeriods = [
        {
            value: "古典时代",
            label: "古典时代",
            description: "约公元前8世纪 - 公元6世纪",
        },
        {
            value: "后古典时代",
            label: "后古典时代",
            description: "约公元6世纪 - 15世纪末",
        },
        {
            value: "近代早期",
            label: "近代早期",
            description: "约15世纪末 - 18世纪末",
        },
                        ...
                        <div className="space-y-6">
                                ...
                                <div className="grid grid-cols-2 gap-2 text-sm">
                                    {timePeriods.map((period) => (
                                        <button
                                            ...
                                            className={`py-3 px-2 rounded-md transition-all duration-200 border ${
                                                selectedTimePeriod ===
                                                period.value
                                ...
                                <div className="grid grid-cols-3 gap-2 text-sm">
                                    {difficulties.map((difficulty) => (
                                        <button
                                            ...
                                            className={`py-3 px-2 rounded-md transition-all duration-200 border flex items-center justify-center ${
                                                selectedDifficulty ===
                                                difficulty.value
                                                    ? "bg-amber-500/20 text-amber-200 border-amber-400/50 ring-2 ring-amber-400"
                                                    : "bg-slate-800/70 hover:bg-amber-500/20 text-slate-200 border-slate-700 hover:border-amber-400/30"
                            ...
                            <button
                                onClick={() => {
                                    ...
                                    window.location.href = `/loading?timePeriod=${encodeURIComponent(
                                        timePeriod
